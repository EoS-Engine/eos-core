    ...
    IPlannerGeneratedEventPublisher plannerGeneratedEventPublisher) : IPlanningClient
{
    public async Task<Plan> SubmitGoalAsync(Goal goal, CancellationToken cancellationToken = default)
    {
        ...
        // §22.1 has no "failed decomposition" state, and Decomposing's only real external I/O
        // dependency (Knowledge/Reasoning, via TaskGraphBuilder) is the one step in this sequence
        // that can genuinely fail. Rather than leaving the Goal stuck in Decomposing indefinitely,
        // resolve the failure via the already-defined §11.6/§22.1 "Cancelled (from any state)"
        // transition — no new state is introduced.
        PlanTask[] tasks;
        try
        {
        ...
        catch
        {
            ...
            // token here would make this call fail immediately, leaving the Goal stuck in
            // Decomposing and replacing the original exception with a fresh cancellation one.
            await goalManager.CancelGoalAsync(decomposing.GoalId, "Goal decomposition failed.", CancellationToken.None);
            throw;
        }

        // §11.4/§10.4: dependency criticality input to the Priority Manager (§11.3) — the count
        // of other Goals depending on this one, via the Dependency Manager's own tracking.
        ...
        var plan = new Plan(
            ...
            // deterministic heuristic: more decomposed steps imply proportionally less confidence
            // in the plan as a whole, in the absence of any frozen document specifying one.
