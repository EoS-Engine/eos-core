...
{
    private static async Task<(DispatchedTaskStore Store, Scheduler Scheduler)> CreateStackAsync()
    {
    ...
    private static async Task<Guid> SeedReadyTaskAsync(DispatchedTaskStore store, Scheduler scheduler)
    {
        await ClearReadyQueueAsync(store);

        var planTask = new PlanTask(Guid.NewGuid(), "Add a logging statement", ["logging"], []);
        var plan = new Plan(Guid.NewGuid(), Guid.NewGuid(), [planTask], 1, 1.0);

        var schedulerWithPlan = new Scheduler(store, new FixedPlanQueryClient(plan), new FixedTierResourceManagementClient(CapacityTier.Safe), concurrencyCeiling: 1_000_000, dailyCapacity: 1_000_000);
    ...
    {
        var (store, scheduler) = await CreateStackAsync();
        await ClearReadyQueueAsync(store);
    ...
    {
        var (store, scheduler) = await CreateStackAsync();
        var taskId = await SeedReadyTaskAsync(store, scheduler);
        var taskStartedPublisher = new RecordingTaskStartedEventPublisher();
        var coordinator = new ExecutionCoordinator(scheduler, store, new AlwaysAllowProtectionClient(), taskStartedPublisher);
    ...
    {
        var (store, scheduler) = await CreateStackAsync();
        await SeedReadyTaskAsync(store, scheduler);
    ...
    public async Task DispatchNextAsync_DispatchesADifferentTask_OnEachSequentialCall()
    {
        ...
        var plan = new Plan(Guid.NewGuid(), Guid.NewGuid(), [lowPlanTask, highPlanTask], 2, 1.0);
        var schedulerWithPlan = new Scheduler(store, new FixedPlanQueryClient(plan), new FixedTierResourceManagementClient(CapacityTier.Safe), concurrencyCeiling: 1_000_000, dailyCapacity: 1_000_000);
