...
### 11.1 External Target Workspace Binding

ADR-010 extends the Local Files and Projects domains to one explicitly selected external **Target Workspace** per engineering invocation. A Target Workspace is the canonical root of the local Git working tree selected by the HumanOperator; it is distinct from the EOS installation/configuration repository. Selection and validation are fail-closed: when an external target was requested, no subsystem may infer, rediscover, substitute, or fall back to the EOS repository. The selected root and project profile form immutable invocation scope; this introduces no persistent workspace registry or project database.

For the initial external-workspace profile, Protection also requires a resolved Git `HEAD` and a clean, stable source state: staged changes, modified tracked files, untracked non-ignored files, and an unborn `HEAD` fail closed; a detached `HEAD` is allowed only when it resolves to a commit. Profile-approved ignored/generated restore assets do not become candidate source state or candidate-editable content. Any such asset that can affect project evaluation, ownership resolution, Gate 1, Gate 2, or isolated validation behavior must have its exact validation input bound before Gate execution: it is either deterministically recreated in isolation from already validated inputs, or its exact content is validated/fingerprinted, copied into the isolated input, and included in the invocation's separate validation-input identity. Otherwise preflight fails closed. Unapproved ignored/generated files remain excluded and irrelevant to validation. The resolved commit plus successful clean/stability validation binds one source-content state across protected preflight, source reads, patch applicability, and establishment of the isolated validation copy. If that source state changes during this sequence, execution fails closed without reconciliation. SeniorEngineer input, applicability input, and isolated Gate source input must be the same state; the separate validation-input identity does not weaken that invariant or introduce a general snapshot system.

Protection applies separately to workspace preflight, bounded manifest discovery, content reads, candidate paths, patch applicability, and isolated Universal Gate inputs. Authorization for a manifest operation does not authorize reading the content of its entries. The canonical absolute selected root may be displayed to the HumanOperator during the local invocation solely to confirm target identity; that display is the only absolute-path visibility exception and does not require persistence of the raw path. Workspace operation paths, manifest paths, role inputs, candidate paths, artifact paths, and all other paths crossing subsystem boundaries must be Target-Workspace-relative, and every resolved operation must remain canonically contained by the selected root. Absolute candidate paths, traversal, ambiguous path forms, and symbolic-link escape are denied. Git administrative content, including `.git/`, is always excluded. Protected secrets, binary content, packages, and generated/build output are excluded from inspection; profile-required build metadata may be inspected only under its explicit preflight or validation authority and is not thereby candidate-editable. Other configuration content requires explicit authorization rather than inheriting access from its presence inside the target.

Manifest discovery is root-contained, profile-aware, metadata-first, deterministic for unchanged workspace state where practical, non-recursive through symbolic links, and bounded by implementation/configuration ceilings. Exceeding a ceiling or being unable to classify a path fails closed. Manifest discovery does not authorize repository indexing, content or embedding ingestion, background scanning, filesystem watching, or durable cataloguing. Candidate evidence is restricted to profile-eligible, Target-Workspace-relative source and test text paths.

Patch applicability executes against the selected canonical Target Workspace. Universal Gates execute against an isolated copy scoped to that same target and its approved project profile; candidate application occurs only in that copy. Gate 2 relevance is determined from the complete affected production-and-test candidate path set: affected test paths directly identify or own relevant test projects, and `NotApplicable` is permitted only when deterministic profile analysis proves that no relevant test project exists. The real workspace, its index, and its Git metadata remain unchanged. The selected Target Workspace and initial SDK-style .NET project profile supply the Projects-domain scope for the invocation; no project registry, plugin mechanism, new Protection engine, approval engine, or role authority is introduced. All other details and limitations are governed by ADR-010.

Workspace-bound execution authority applies only to tasks explicitly associated with the current authorized invocation and its Plan. Persisted Ready tasks from another invocation, Plan, Goal, or unknown context are ineligible and do not inherit the current Target Workspace authority; an incomplete external-workspace task must not later execute under an unrelated invocation's workspace context. This association may use existing Plan/Task identity and invocation context and requires neither a new task lifecycle state nor Target Workspace fields on every persisted entity.

External Gate 1–2 authorization additionally requires the HumanOperator to authorize the selected workspace as trusted for local execution of project-controlled build and test code. Filesystem-path selection alone is insufficient. Before execution, the HumanOperator must be informed that project code may use host filesystem and network capabilities, that the isolated copy protects workspace mutation rather than host execution, and that credential scrubbing is not an operating-system sandbox. This rule introduces no new approval subsystem and does not prescribe exact command-line interaction.
