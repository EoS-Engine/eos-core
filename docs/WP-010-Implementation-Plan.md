...
12. **(Post-PR, CodeRabbit review on PR #7)** Five VALID findings fixed, one VALID finding partially deferred pending authorization:
    - **Model-aware adapter routing:** `AIProviderManager`'s adapter lookup was keyed by provider name only, and `Program.cs` built one adapter per provider from `Models[0]`, silently ignoring `InferenceRouter`'s per-request model selection for any provider configured with more than one model. Fixed by keying `adapters` on `(ProviderName, ModelName)` and building one adapter per `provider.Models` entry in `Program.cs` (reusing each provider's shared `HttpClient`). No public contract, dependency, or Constitution impact — confined to `AIProviderManager`'s internal constructor parameter type and `Program.cs`'s adapter-construction loop.
    - **Correlation IDs + per-attempt failure logging:** `request.CorrelationId` (already in scope on every call) was never included in `InferenceRouted`/`InferenceCompleted` log lines, and a candidate that failed below `HealthMonitor`'s failure threshold produced no log output at all. Fixed by including the correlation ID in both existing log lines and adding an `InferenceAttemptFailed` warning per failed candidate, per Constitution Part 5's correlation-ID-propagation requirement (line 678).
    - **Empty nested config collections (partial):** `ProviderEntry.Models` and `ModelEntry.Capabilities` had `[Required]` only (blocks `null`, not `[]`). Added `[MinLength(1)]` to both — confined to `ProvidersOptions.cs` (an approved file). The related ask — recursing `JsonConfigurationLoader`'s validation into nested `ModelEntry` items — touches a file explicitly outside WP-010's approved scope and was **not applied**; flagged for separate authorization.
